...
public class WebViewPGsHandler implements HttpHandler {
    ...
    private List<String> fetchPGsFromDB(String partnerId) throws SQLException {
        ...
        // Explicitly selecting all 24 required columns in order
        String sql =
            ...
            "room_price, amenities, policies, rating, pg_contact, about_this_pg, pg_images, status " +
        ...
        try (Connection conn = dbConfig.getPartnerDataSource().getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    // Loop through the 25 columns (1 to 25)
                    // Note: We selected 25 columns in the SQL above
                    for (int i = 1; i <= 25; i++) {
